document.addEventListener('DOMContentLoaded', () => {
    ...
    let state = {
        ...
        currentTheme: 0,
        themes: ['', 'theme-sunset', 'theme-ocean', 'theme-forest'],
        quotes: [
    ...
    // --- PROFILE DROPDOWN & LOGIN MODAL ELEMENTS ---
    const profileBtn = document.getElementById('profile-btn');
    const profileDropdown = document.getElementById('profile-dropdown');
    const dropdownLoginBtn = document.getElementById('dropdown-login-btn');
    const dropdownStreakBtn = document.getElementById('dropdown-streak-btn');
    const dropdownAchievementsBtn = document.getElementById('dropdown-achievements-btn');
    ...
    const navigateTo = (hash) => {
        // Hide achievement view if it's currently active before switching
        const achievementsView = document.getElementById('achievements-view');
        if (achievementsView.classList.contains('active')) {
             achievementsView.classList.remove('active');
        }
    ...
    loginForm.addEventListener('submit', (e) => {
        ...
        if (name) {
            ...
            dropdownLoginBtn.textContent = `Welcome, ${state.user.name}`;
    ...
    dropdownStreakBtn.addEventListener('click', (e) => {
        e.preventDefault();
    ...
    dropdownAchievementsBtn.addEventListener('click', (e) => {
        e.preventDefault();
    ...
    createGroupForm.addEventListener('submit', (e) => {
        ...
        const members = [state.user.name, ...membersStr.split(',').map(m => m.trim()).filter(m => m)];
        const newGroup = {
            id: 'group' + Date.now(),
            name: name,
            members: members
